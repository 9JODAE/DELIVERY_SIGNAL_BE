...
public class StockSearchFacade {
	...
	public Page<StockResult> searchStocks(SearchStockCommand command) {
	 ...
	 */
	public Page<StockResult> searchStocksFallback(SearchStockCommand command, Throwable throwable) {
	...
	private Page<StockResult> mapToResults(Page<Stock> stocks, Map<UUID, ProductInfo> productInfos) {
		if (stocks.isEmpty()) {
			return Page.empty();
		}
	...
	private StockResult createStockResult(Stock stock, Map<UUID, ProductInfo> productInfos) {
		UUID productId = stock.getProductId().getValue();
		ProductInfo productInfo = productInfos.get(productId);
		return productInfo != null ? StockResult.from(stock, productInfo) : null;
	}

	private Page<StockResult> mapToResults(Page<Stock> stocks) {
		if (stocks.isEmpty()) {
			return Page.empty();
		}

		List<StockResult> results = stocks.stream()
			.map(stock -> StockResult.of(
				stock.getStockId(),
