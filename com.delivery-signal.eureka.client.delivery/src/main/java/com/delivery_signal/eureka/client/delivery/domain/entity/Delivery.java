...
public class Delivery extends BaseEntity {
    ...
    // 배송 전체 상태를 최신 경로 기록 상태와 동기화
    public void syncStatusWithRoute(DeliveryStatus latestRouteStatus, Long updaterId) {
        // Delivery Route의 최종 상태가 HUB_ARRIVED가 아니라면 Delivery의 전체 상태를 동기화
        if (latestRouteStatus.ordinal() < DeliveryStatus.HUB_ARRIVED.ordinal()) {
            this.currStatus = latestRouteStatus;
            super.update(updaterId);
        } else if (latestRouteStatus == DeliveryStatus.HUB_ARRIVED && this.currStatus != DeliveryStatus.HUB_ARRIVED) {
            // 경로 기록의 최종 상태가 목적지 허브 도착이면, Delivery 상태도 도착으로 변경
            this.currStatus = DeliveryStatus.HUB_ARRIVED;
            super.update(updaterId);
        }
    }
    ...
    public void updateStatus(DeliveryStatus newStatus, Long updatorId) {
        ...
        // HUB_MOVING에서 DELIVERING으로의 전이만 허용
        if (this.currStatus.equals(DeliveryStatus.HUB_MOVING) && newStatus.equals(DeliveryStatus.DELIVERING)) {
