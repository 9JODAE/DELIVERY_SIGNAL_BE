...
- 주문 상품(OrderProduct) 등록 / 조회 / 수정 / 삭제 처리
- 주문 상태 기반 흐름 제어 (주문생성 → 재고차감 -> 배송생성)
- 주문 취소 시 연관 상품 상태 및 재고 복원 처리
- DDD 구조 적용
  - OrderService → 비즈니스 흐름 제어
  - OrderDomainService → 도메인 규칙 및 검증 로직 담당
  - OrderQueryPort, OrderCommandPort → Repository 접근 추상화
- 의존성 누수 해결
  - OrderRepository 직접 의존 제거
  - Port/Adapter 패턴으로 인프라 접근 단일화
- 주문·상품 엔티티별 상태 관리 분리
  - Order 엔티티: 전체 주문 단위 상태 관리
  - OrderProduct 엔티티: 개별 상품 단위 상태 관리
### 배송 (Delivery)
- 배송 등록 /조회 / 수정 / 삭제 처리
...
### 주문 - DDD 적용 시 의존성 누수
- 문제
  - DDD 구조를 적용했지만, OrderService가 OrderRepository를 직접 의존하면서 인프라 계층에 대한 간접 의존이 발생함.

- 원인
  - DDD를 단순히 계층 import 제약으로만 이해
  - Repository 인터페이스가 도메인에 있어도 구현체는 인프라에 존재 → 간접 의존 성립 
  - Port/Adapter 원칙이 깨짐

- 해결
  - OrderRepository 접근 로직을 OrderQueryPort, OrderCommandPort로 분리 
  - Application 계층은 비즈니스 흐름 제어만 담당 
  - 도메인 로직은 OrderDomainService, 영속성은 infrastructure로 이동
