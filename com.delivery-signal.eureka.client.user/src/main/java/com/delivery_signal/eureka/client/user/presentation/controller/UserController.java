...
import com.delivery_signal.eureka.client.user.application.exception.UnauthorizedAccessException;
...
//    public ResponseEntity<ApiResponse_delete<GetUserRoleResponse>> confirmUserRole(@RequestHeader("x-user-id") String x_user_id, @RequestBody CheckUserRoleRequest requestDto) {
//        // 로그인한 사용자의 권한 확인 아닌 요청 body의 사용자(userId)에 대한 권한 확인
//        Boolean check = userService.checkUserRole(requestDto);
//        if (!check) {
//            return ResponseEntity.status(HttpStatus.OK).body(ApiResponse_delete.message("권한이 일치하지 않습니다"));
//        }
//        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse_delete.success(new GetUserRoleResponse(requestDto.userId(), requestDto.role(), null)));
//
//    }
...
//    @PreAuthorize("hasRole('MASTER')")
    ...
    public ResponseEntity<ApiResponse<GetUserResponse>> registerUser(@RequestHeader("x-user-id") String x_user_id, @RequestBody CreateUserRequest requestDto) {
        ...
        isMaster(Long.parseLong(x_user_id));
...
//    @PreAuthorize("hasRole('MASTER')")
    ...
    public ResponseEntity<ApiResponse<GetUserResponse>> findUser(@RequestHeader("x-user-id") String x_user_id, @PathVariable("userId") Long userId) {
        ...
        if (!isMaster(Long.parseLong(x_user_id))) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse_delete.message("특정 사용자 조회는 MASTER만 가능합니다"));
        }

        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse_delete.success(new GetUserResponse(
                2L,
                "kimhubgwan",
                "UKIMHUBGWAN",
                "서울특별시 센터 허브",
...
//            @Parameter(description = "사용자 이름, 사용자 소속 그룹(허브 또는 업체)명, 권한으로 검색 가능")
        ...
        if (!isMaster(Long.parseLong(x_user_id))) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse_delete.message("사용자 전체 조회는 MASTER만 가능합니다"));
        }

        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse_delete.success(List.of(
...
//    @PreAuthorize("hasAnyRole('MASTER, HUB_MANAGER')")
    ...
    public ResponseEntity<ApiResponse<Void>> permitUser(@RequestHeader("x-user-id") String x_user_id, @RequestBody UpdateUserApprovalStatusRequest requestDto, @PathVariable("userId") Long userId) {
        ...
        try {
            ...
            try {
                isHubManager(Long.parseLong(x_user_id));
            } catch (UnauthorizedAccessException e2) {
                throw new UnauthorizedAccessException("MASTER 또는 HUB_MANAGER");
            }
        }
...
//    @PreAuthorize("hasRole('MASTER')")
    ...
    // Master 여부 확인
    private void isMaster(Long userId) {
        if (!(userService.checkUserRole(new CheckUserRoleRequest(userId, UserRoleType.MASTER)))) {
            throw new UnauthorizedAccessException("MASTER");
        }
    }

    // Hub Manager 여부 확인
    private void isHubManager(Long userId) {
        if (!(userService.checkUserRole(new CheckUserRoleRequest(userId, UserRoleType.HUB_MANAGER)))) {
            throw new UnauthorizedAccessException("HUB_MANAGER");
        }
    }
